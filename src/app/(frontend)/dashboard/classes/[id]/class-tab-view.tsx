...
export function ClassTabView({ classId }: ClassTabViewProps) {
    const [students, setStudents] = React.useState([
        { id: '1', firstName: 'Alice', lastName: 'Smith', dateOfBirth: '2019-05-12' },
        { id: '2', firstName: 'Bob', lastName: 'Johnson', dateOfBirth: '2019-03-22' },
    ]);
                    ...
                    <TabsContent value="students">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-4">
                            {students.map((student) => (
                                <Link key={student.id} href={`/dashboard/children/${student.id}`}>
                                    <div className="border p-3 rounded flex gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">Born: {student.dateOfBirth}</p>
                                        </div>
                                    </div>
                                </Link>
                            ))}
                            ...
                            {teachers.map((teacher) => (
                                <Link key={teacher.id} href={`/dashboard/staff/${teacher.id}`}>
                                    <div className="border p-3 rounded flex gap-3">
                                        ...
                                        <div>
                                            <p>{teacher.name} {teacher.isLead && <Badge>Lead</Badge>}</p>
